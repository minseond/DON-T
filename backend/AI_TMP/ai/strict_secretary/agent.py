...
from consumption_poc.config import get_env, DEFAULT_GEMINI_MODEL
from consumption_poc.ai_insights import _model_candidates, _extract_json
from strict_secretary.models import SecretaryResponse
...
""".strip()
        ...
        for candidate in _model_candidates(self.model_name):
            try:
                ...
                if "is_approved" in parsed and "fact_violence_comment" in parsed and "reasoning" in parsed:
                    return SecretaryResponse(
                        is_approved=bool(parsed["is_approved"]),
                        fact_violence_comment=str(parsed["fact_violence_comment"]),
                        reasoning=list(parsed["reasoning"]),
                    )
            except Exception as e:
                import logging
                logging.warning(f"Error evaluating purchase with model {{candidate}}: {{e}}")
                continue
